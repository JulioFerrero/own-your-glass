...
# apps.sh — hide a curated list of unwanted apps from the launcher.
#
# The supported mechanism (verified on the device)
# -----------------------------------------------
# LG's webOS application manager reads
#   /var/preferences/com.webos.applicationManager/blockedSystemAppList/<REGION>.json
# where <REGION> is the device's two-letter region code (ESP for Spain
# on this device). The file is a small JSON object with one array:
#
#   {"blocked_system_applist":[
#     "com.webos.app.buddy",
#     "com.webos.app.gamehome",
#     "com.webos.app.lifealarm",
#     ...
#   ]}
#
# Apps whose IDs appear in this array are HIDDEN from the launcher —
# not deleted, not killed, just not offered to the user. webOS reads
# this file at every launcher render, so the effect is immediate AND
# survives any app-manager housekeeping that re-writes the file.
#
# /var/preferences is writable on this device (confirmed). The region
# directory may contain zero or more region files; we discover the
# right one dynamically by globbing blockedSystemAppList/*.json and
# preferring any existing file (so we touch whatever the app manager
# has already created for THIS region).
#
# Implementation strategy
# -----------------------
# 1. Discover the target file: glob blockedSystemAppList/*.json; if
#    none exist, pick a deterministic placeholder name so we can
#    still create a valid file (the app manager will accept it on
#    next launch — verified shape).
# 2. Back the existing file up ONCE to
#      $OYG_BACKUP/blockedSystemAppList.<region>.json
#    (never overwrite; a later edit cannot quietly corrupt the audit
#    trail).
# 3. Read existing blocked IDs (if any), merge with the curated list,
#    filter curated IDs to those that ACTUALLY exist on the device
#    (under /usr/palm/applications/<id> or /media/system/apps/<id>),
#    sort + dedupe, write back via temp file + mv.
# 4. NO python on the device — we use only awk, sed, tr, sort, uniq,
#    and grep. The JSON shape is one small object; we control both
#    ends (the existing file is generated by LG with a fixed layout,
#    and the curated list is our own). The rewrite is therefore
#    safe: emit exactly
#      {"blocked_system_applist":["id1","id2",...]}
#    with deterministic ordering, regardless of the input layout.
#
# Skip-if-absent (portability)
# ----------------------------
# webOS TV builds differ. A curated ID may simply not exist on this
# build. The module checks /usr/palm/applications/<id> and
# /media/system/apps/<id> — the two canonical locations — and skips
# absent IDs with a clear warning. Status reports how many of the
# curated IDs were added vs skipped, so the operator can spot drift.
#
# WHAT THIS MODULE DOES NOT DO
# ----------------------------
# - It HIDES apps from the launcher; it does not delete them. Apps
#   are still installed on the filesystem and can be re-shown by
#   removing their IDs from the blocked list.
# - It does not remove STORE-DELIVERED promotional tiles (e.g.
#   "Rakuten TV", "LG Streaming Week"). Those are not local apps —
#   they are pulled from the content-store / recommendation feed
#   by the content manager. The only effective countermeasure for
#   that class is the network blocklist (the `network` module's
#   hosts-file sinkhole), which is documented separately in
