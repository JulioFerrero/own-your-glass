...
- **No `/etc/resolv.conf` direct edit.** `/etc/resolv.conf` is a
  symlink to `/var/lib/misc/resolv.conf`, which IS writable, and the
  network module's layer 2 can install an override there behind the
  `OYG_DNS_OVERRIDE=1` flag (default-gateway as nameserver).
...
| `/usr/sbin/sdx` | luna-launched binary (no systemd unit) | Software-delivery daemon (over-the-air content / store-tile updates). Neutralised the same way. **Kills LG's content-update channel** — the device still launches installed apps but stops receiving new / updated content tiles. |
